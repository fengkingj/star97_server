...
/*
 * C# Version
 * public static byte[] MsgCompress(byte[] _msg)
    {
        if (_msg.Length == 0) return new byte[0];
        List<byte> lisRes = new List<byte>();
        int headIndex = 0;
        lisRes.Add(0);
        int bitPos = 7;
        for(int i=0;i<_msg.Length;++i)
        {
            if(_msg[i] > 0)
            {
                lisRes[headIndex] = (byte)(lisRes[headIndex] |(0x01 << bitPos));
                lisRes.Add(_msg[i]);
            }
            bitPos--;
            if(bitPos == -1 && bitPos < _msg.Length-1)
            {
                bitPos = 7;
                headIndex = lisRes.Count;
                lisRes.Add(0);
            }
        }
        return lisRes.ToArray();
    }
    public static byte[] MsgUncompress(byte[] _msg)
    {
        if (_msg.Length == 0) return new byte[0];
        List<byte> lisRes = new List<byte>();
        int headIndex = 0;
        while (headIndex < _msg.Length)
        {
            byte mark = _msg[headIndex];
            for (int i = 7; i >= 0; --i)
            {
                if (((mark >> i) & 0x01) == 1)
                {
                    headIndex++;
                    lisRes.Add(_msg[headIndex]);
                }
                else
                {
                    lisRes.Add(0);
