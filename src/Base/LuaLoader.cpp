...
bool LuaLoader::FileModified(string& _file)
{
	if(!_instance)
	{
	...
	struct stat buf;
	int res = stat(_file.c_str(),&buf);
	if(res != 0)
	{
		_log(_ERROR,"LuaLoader","FileModified Problem getting file info. File=%s",_file.c_str());
		return NULL;
	}
	long long filetime = (long long)buf.st_mtime+(long long)buf.st_ctime;
	unordered_map<string,Loader>::iterator it=_instance->_lua_loaders.find(_file);
	if(it==_instance->_lua_loaders.end())
	{
		return true;
	}
	else if(filetime != it->second._last_time)
	{
		return true;
	}
	return false;
}
LuaConfig* LuaLoader::LoadFile(string& _file)
{
	if(!_instance)
	{
		_instance = new LuaLoader();
	...
	if(res != 0)
	{
		_log(_ERROR,"LuaLoader","LoadFile Problem getting file info. File=%s",_file.c_str());
		return NULL;
	}
	long long filetime = (long long)buf.st_mtime+(long long)buf.st_ctime;
	unordered_map<string,Loader>::iterator it=_instance->_lua_loaders.find(_file);
	if(it!=_instance->_lua_loaders.end())
	{
		it->second._success = it->second._lc.LoadFile(_file.c_str());
	...
	{
		Loader* ld = &(_instance->_lua_loaders[_file]);
		ld->_last_time = filetime;
		ld->_success = ld->_lc.LoadFile(_file.c_str());
		if(ld->_success)
		{
			return &(ld->_lc);
		}
